...
public class CommandRemove extends AbstractCommand implements CommandExecutor {
    @Inject
    ...
    @Inject
    public CommandRemove(LocaleService localeService, ServiceManager serviceManager) {
        this.localeService = localeService;
        this.serviceManager = serviceManager;

        alias("remove", "r", "rm", "del", "delete");
            ...
            .ifPresent(
                economyService ->
                    commandSpec(
                        CommandSpec
                            .builder()
                            .permission("multicurrencies.command.remove")
                            .description(localeService.getTextWithFallback("command.multicurrencies.subcommand.remove.description"))
                            .executor(this)
                            .arguments(
        ...
        serviceManager
            ...
            .ifPresent(
                economyService -> {
                    ...
                    if (account.withdraw(currency, amount, Cause.of(EventContext.empty(), pluginContainer)).getResult().equals(ResultType.SUCCESS)) {
                        Text removeMessage = localeService.getTextWithFallback(
                            "info.removeBalance",
                            Tuple.of("currency", currency.getDisplayName()),
                            Tuple.of("amount", amount),
                            Tuple.of("symbol", currency.getSymbol())
                        );
                        src.sendMessage(removeMessage);
                        src.sendMessage(
                        ...
                        if (args.hasAny("m")) {
                            user.getPlayer().ifPresent(player -> player.sendMessage(removeMessage));
                        }
                    } else {
